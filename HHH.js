// setup
for (c.width = c.height = W = 510, a.font = "3em m", z = j = 20, f = 255, n = k = v = u = i = I = S = 0, T = 10, l = [], o = [], p = Math, q = 2 * p.PI, r = p.random, s = p.sqrt, t = p.abs, P=p.pow,
...
Z = function(d, b, g, e, w) {
    ...
    for (u = 0, K = f + g * k , J = f + e * k ; n = l[u++]; )
        ...
        if (1 ^ n) {
            ...
            if (!j) {
                ...
                // get acceleration
                v = 40 < s( P(n.x-f,2) + P(n.y-f,2) ) ? -.15 : .4;

                // get velocity and position
                n.x += n.a += v/2 * (n.x > f ? 1 : -1) - 1e-4;
                n.y += n.b += v/2 * (n.y > f ? 1 : -1);
    ...
    // draw our hippo
    A( K + g * 65, J + e * 65, 50, j + 1)(K + g * z, J + e * z, 60)
				

    // draw the score
    ( w , K + g * 40 - T, J + e * 40 -T , 9 , 1 )
